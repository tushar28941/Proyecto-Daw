...
if(isset($_POST["nombre"])){
    ...
    if($nombre==""){
        ...
        if(!mysqli_query($conn, $sql)){
            array_push($errores, "Error al insertar");
        ...
        window.onload = function() {
            ...
            document.getElementById("borrar").onclick = function () {
                if (confirm("¿Eliminar el producto?")){
                    var id = <?php print $id; ?>;
                    window.open("graficas.php?m=B&id="+id,"_self");
                }                
            }
            <?php } ?>
            ...
            <?php if($m=="S") { ?>
				...
				window.open("graficas.php?m=A","_self");
			}
            <?php } else { ?>
            document.getElementById("volver").onclick = function () {
                window.open("graficas.php","_self");
            }
            <?php } ?>
    ...
    <div class="container-fluid text-center">
        <div class="row content">
            ...
            <?php if($m=="S") { ?>
                ...
                <input type="button" name="alta" id="alta" value="Añadir una tarjeta gráfica" class="btn btn-info" role="button">
            <?php } ?>
            </div>
            <div class="col-sm-8 text-left">
                <div class="card-body">                
                <h2 class="text-center">Tarjetas Gráficas</h2>
                <?php 
                if($m=="A" || $m=="C"){
                    if(count($errores)>0){
                ...
                ?>
                    ...
                    if ($m=="S") {
                        ...
                        for ($i=0; $i < count($productos) ; $i++) { 
                            ...
                            print '<p><a href="graficas.php?m=C&id='.$productos[$i]["id"].'">'.$productos[$i]["nombre"].'</a></p>';
                            print '</div>';
                            $ren++;
                            if ($ren==4) {
                                $ren = 0;
